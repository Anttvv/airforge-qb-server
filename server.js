...
const QB_REDIRECT_URI = process.env.QB_REDIRECT_URI || 'https://airforge-qb-server-production.up.railway.app/callback';
...
// Health check
app.get('/', (req, res) => {
  res.json({ status: 'AirForge QB server running' });
});
...
app.get('/callback', async (req, res) => {
  ...
  try {
    ...
    await sb.from('qb_connections').upsert({
      company_id, realm_id: realmId, access_token, refresh_token, expires_at,
...
app.post('/qb/invoice', async (req, res) => {
  ...
  try {
    ...
    const custSearch = await axios.get(
      `${baseUrl}/query?query=select * from Customer where DisplayName = '${customerName.replace(/'/g, "\\'")}'&minorversion=65`,
      { headers }
    );
    ...
    if (existingCust) {
      ...
      const newCust = await axios.post(`${baseUrl}/customer?minorversion=65`, {
        DisplayName: customerName,
      }, { headers });
      qbCustomerId = newCust.data.Customer.Id;
    }
    // Build line items
    const laborAmount = (job.total_charged || 0) - (job.parts_cost || 0);
    const lineItems = [{
      Amount: laborAmount > 0 ? laborAmount : job.total_charged || 0,
      DetailType: 'SalesItemLineDetail',
      Description: `${job.job_type}${job.technicians?.name ? ' — ' + job.technicians.name : ''}`,
      SalesItemLineDetail: { UnitPrice: laborAmount > 0 ? laborAmount : job.total_charged || 0, Qty: 1 }
    }];
    if (job.parts_cost > 0) {
      lineItems.push({
    ...
    const invoiceRes = await axios.post(`${baseUrl}/invoice?minorversion=65`, {
      Line: lineItems,
...
app.get('/qb/status', async (req, res) => {
  const { company_id } = req.query;
  if (!company_id) return res.status(400).json({ error: 'company_id required' });
  const { data: conn } = await sb.from('qb_connections').select('connected_at,realm_id').eq('company_id', company_id).maybeSingle();
